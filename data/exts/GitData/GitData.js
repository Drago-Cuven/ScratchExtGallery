...
(function (Scratch) {
  ...
  class GitDataExtension {
    ...
    async _fetchFileContentAndBase64(fileUrl) {
      try {
        ...
        const data = await response.json();
        if (!data.content) return { content: null, base64: null };
        return {
          content: data.encoding === "base64" ? atob(data.content) : data.content,
          base64: data.encoding === "base64" ? data.content : null
        };
      } catch (error) {
        console.error("Git Data: Failed to fetch file content:", error);
        return { content: null, base64: null };
      }
    }

    async _getFileData(path) {
      const fileInfo = this._getFileInfo(path);
      if (!fileInfo || !fileInfo.url) {
        return { content: null, base64: null };
      }
      return await this._fetchFileContentAndBase64(fileInfo.url);
    }

    async _getFileContent(path) {
      const { content } = await this._getFileData(path);
      return content;
    }

    async _getFileBase64(path) {
      const { base64 } = await this._getFileData(path);
      return base64;
    }

    async _getFileDataUrl(path) {
      const base64 = await this._getFileBase64(path);
      if (!base64) return "";
      ...
      const mimeMap = {
        ...
        mp4: "video/mp4", pdf: "application/pdf", md: "text/markdown", xml: "application/xml"
      };
        ...
        blocks: [
          {
            ...
            text: this.showCategory.repository ? translate("Repository") + " ▼" : translate("Repository") + " ▶",
          },
          {
          ...
          {
            opcode: "doesRepositoryExist",
            blockType: BlockType.BOOLEAN,
            text: translate("does repository [REPO] exist?"),
            arguments: {
              REPO: {
                type: ArgumentType.STRING,
                defaultValue: "https://github.com/username/repository",
...

          {
            ...
            blockType: BlockType.BUTTON,
            text: this.showCategory.directory ? translate("Directory") + " ▼" : translate("Directory") + " ▶",
          },
          {
            blockType: BlockType.LABEL,
...

          {
            ...
            blockType: BlockType.BUTTON,
            text: this.showCategory.files ? translate("Files") + " ▼" : translate("Files") + " ▶",
          },
          {
            blockType: BlockType.LABEL,
    ...
    async doesRepositoryExist({REPO}) {
      const repoUrl = Cast.toString(REPO);
      const repoPath = this._parseGithubUrl(repoUrl);
    ...
    async readFile({filename, format}) {
      ...
      if (formatType === "dataurl") {
        return await this._getFileDataUrl(resolvedPath);
